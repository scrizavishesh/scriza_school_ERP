...
import { Conatct_comission_GetById } from '../Utils/Apis'
...
const Contac_commis = ({ data }) => {
  ...
  const [isValidCommissionOptionRequired, setIsValidCommissionOptionRequired] = useState(false);
  ...
  const FuncValidation = () => {
    ...
    // allowance
    if (commissionOption === "" || !commissionOption) {
      setIsValidCommissionOptionRequired(true)
    }
    else {
    }
    // title 
  ...
  const handleallowance = (e2) => {
    setommissionOption(e2)
    const noRegex = /^[A-Za-z\s]+$/;
    setIsValidCommissionOptionRequired(noRegex.test(e2));
    if (e2 === "" || !noRegex.test(e2)) {
      setIsValidCommissionOptionRequired(true)
    } else {
      setIsValidCommissionOptionRequired(false)
    }
  }
  // title 
  ...
  const ContactDataApi = async () => {
    if (FuncValidation()) {
      ...
      setLoader(true)
      try {
        const response = await UserCommissionGetAllApi(MyUserID, formData);
  ...
  const MyStaffGetById = async () => {
    ...
    setLoader(true);
    try {
      const response = await Conatct_comission_GetById(MyUserID);
      console.log("my commission get by id api is here______________ ", response);
      if (response?.status === 200) {
        // toast.success(response?.data?.msg);
        setUpdateStatus(response?.data?.status);

        setommissionOption(response?.data?.commission?.commissionOption);
        setTtile(response?.data?.commission?.title);
        setAmountOption(response?.data?.commission?.amountOption);
        setAmount(response?.data?.commission?.amount);
          ...
          <div className="row px-3">
            <div className="col-lg-6 col-md-6 col-sm-12 ">
              <div className="mb-3  pt- for-media-margin">
                ...
                <input type="text" className="form-control form-focus-input form-control-sm heading-14 grey-input-text-color input-border-color" value={commissionOption} onChange={(e) => handleallowance(e.target.value)} style={{ borderRadius: '5px', marginTop: '-5px' }} id="exampleFormControlInput12" placeholder="Enter Title" />
              </div>
              <div className='pt-1'>
                {isValidCommissionOptionRequired && (
