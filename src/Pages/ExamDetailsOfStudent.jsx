...
import { getExamDetailsByIdApi } from '../Utils/Apis';
import DataLoader from '../Layouts/Loader';

const ExamDetailsOfStudent = ({ StudentId }) => {
    ...
    const getExamsByStudentId = async () => {
        try {
            setloaderState(true);
            var response = await getExamDetailsByIdApi(StudentId);
            if (response?.status === 200) {
                if (response?.data?.status === 'success') {
                    setloaderState(false)
                    setExamsData(response?.data?.marks)
                }
                else {
                    console.log('error')
                    setloaderState(false)
...
export default ExamDetailsOfStudent
